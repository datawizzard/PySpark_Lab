...
if __name__ == '__main__':
    ...
    df3 = df2.groupBy(window(col("CreatedTime"),"15 minute"))\
    .agg(sum("BUY").alias("Total_Buy"),
         sum("SELL").alias("Total_Sell"))
    df4 = df3.select("window.start","window.end","Total_Buy","Total_Sell")
    results = df3.writeStream.format("console").outputMode("update").option("truncate", "false") \
        .option("checkpointLocation", "/home/saif/Desktop/checkpoint/c6").start()
